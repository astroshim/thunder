		...
		,m_iDay(0)
{
	...
	memset(m_pchOriginFName, 0x00, sizeof(char)*128);
}
...
{
	//m_iDay = CNPUtil::GetCurDay();	// 날짜 단위로 로그를 남긴다.
	m_iDay = CNPUtil::GetTime();		// 시간 단위로 로그를 남긴다.

	char cur_date[20], cur_time[20];
	CNPUtil::GetTime(cur_date, cur_time);

	if(m_pchOriginFName[0] == 0x00)
	{
		memset(m_pchOriginFName, 0x00, sizeof(char)*128);
		strcpy(m_pchOriginFName, _pchFileName);
	}

	memset(m_pchFileName, 0x00, sizeof(char)*MAX_FILE_NAME);
	//strncpy(m_pchFileName, _pchFileName, MAX_FILE_NAME);
	snprintf(m_pchFileName, MAX_FILE_NAME, "%s_%s_%s.log", m_pchOriginFName, cur_date,cur_time);
...
*	시간단위로 로그를 남긴다
*/
	if(m_iDay != CNPUtil::GetTime() || 
		FileSize(m_pchFileName) > MAX_LOG_FILE_SIZE)
	{
		m_iDay = CNPUtil::GetTime();
	}
	else
	{
		return;
...
*/
	...
	SetFileName(m_pchOriginFName);
	//unlink(m_pchFileName);
	//BackupLogFile();
	//OpenFd();
}
...
{
	char logdname[PATH_MAX], renfname[PATH_MAX];
	...
	if( (rst = stat(logdname, &st)) < 0 )
	{
		...
		if( mkdir(logdname, S_IRWXU | S_IRWXG | S_IRWXO) < 0 )
		{
			return ;
		}
	}
	...
	sprintf( renfname, "%s/%s.%02d%02d%02d", logdname, basename(orifname),
			tm->tm_year % 100,
			tm->tm_mon + 1,
			tm->tm_mday);

	int iLogFileCnt;
	if( (iLogFileCnt = FileCount(logdname, basename(renfname))) > 0)
	{
...
const int CNPLog::FileCount(const char* const _pchDirName, const char* const _pchFileName)
{
    ...
    if (dp != NULL) 
	{
        for(;;) 
		{
			...
			if(strncmp(item->d_name, _pchFileName, strlen(_pchFileName)) == 0)
			{
				cnt++;
			}
        }
